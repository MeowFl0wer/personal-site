...
/**
 * Turns on demo mode in the throwaway database.
 *
 ...
 * built for. It reads a setting; this decides what that setting says, in a
 * database that exists for the length of this build.
 */
const enableDemoMode = () => {
  if (!fs.existsSync(DB_FILE)) return;
  const result = spawnSync("sqlite3", [DB_FILE, "update site_settings set demo_mode = 1;"], {
    encoding: "utf8",
  });
  if (result.status !== 0) {
    throw new Error(
