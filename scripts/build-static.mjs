...
/**
 * Ctrl-C has to leave the checkout as it found it: the server-only routes back
 * in the tree, the demo flag back as it was.
 *
 * One handler, and it does nothing but turn the interrupt into an ordinary
 * exit. The cleanups below ride on `exit`, which Node runs for every listener
 * however the process ends — where two SIGINT handlers would race, and the
 * first one to call `process.exit` would keep the second from ever running.
 */
...
/** One statement against the database this build reads. */
const sqlite = (statement) => spawnSync("sqlite3", [DB_FILE, statement], { encoding: "utf8" });

/** `0`, `1`, or `null` for a row that has never had the box ticked either way. */
const readDemoMode = () => {
  const result = sqlite("select coalesce(demo_mode, 'null') from site_settings limit 1;");
...
/**
 * Runs `fn` with demo mode on, and puts the setting back exactly as it was.
 *
 * The preview is a demonstration and has to say so — and the half of /about
 * that is private on the real site has nothing to protect here, so it becomes
 * something a visitor can switch on and off instead.
 *
 * Set here rather than read from an environment variable inside the
 * application, because `src/` does not get to know which target it is being
 * built for. It reads a setting; this decides what that setting says.
 *
 * The restoring is the point. In CI the database is seeded for this build and
 * thrown away after it, so it never mattered there — but run locally this
 * writes to the working copy in `data/`, and leaving the flag on turned the
 * next `next dev` into the demonstration: the notice on arrival, the private
 * half of About unlocked for anyone. Someone checking the real site then sees
 * the preview and has no reason to suspect the build they ran an hour ago.
 * So it goes back, on failure and on Ctrl-C as well.
 */
const withDemoMode = async (fn) => {
  const before = readDemoMode();

  /* Called from the `finally` and again on the way out if that never ran. */
  let done = false;
  const restore = () => {
    if (done || before === undefined) return;
    done = true;
    sqlite(`update site_settings set demo_mode = ${before === "null" ? "null" : before};`);
    log(`demo mode restored to ${before}`);
  };

  process.on("exit", restore);

  if (sqlite("update site_settings set demo_mode = 1;").status !== 0) {
    process.off("exit", restore);
    throw new Error(
      "Could not turn on demo mode.\nRefusing to publish a preview that does not say it is one.",
    );
  }
  log("demo mode on for the preview");
